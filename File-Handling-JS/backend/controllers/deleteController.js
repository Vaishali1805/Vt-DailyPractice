...
export function deleteAllFiles(req, res) {
  console.log("am in deleteAllFiles")
  const { selectedFiles } = JSON.parse(req.body);
  ...
  myCache.set("uploadedFiles", updatedFiles);
      ...
      fs.access(filePath, fs.constants.F_OK, (err) => {
        ...
        fs.unlink(filePath, (err) => {
          if (err) {
            ...
            res.json({ message: "Files deleted successfully", files: updatedFiles });
          }
        });
      })
    }
